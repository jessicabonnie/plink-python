...
OBJECT Options:
  ...
  [-t] ----->       assoc           Case/Control for QTL association
    ...
    '''
    ).format(OUTPUT_FOLDER,OUT_FLAG,CHROMOSOME,REFGENE,CHRBAND,
...
def cl_arguments(argv):
    ...
    global condition_list,c_interval, chrband, build, interrupt
    ...
    try: 
        opts, args = getopt.getopt(argv, "ho:s:f:t:c:p:l:g:",
				    ...
				    "from-bp=","to-bp=","condition-list=",
                                    "pheno=","ci=","chrband=","build=","interrupt"])
...
def main(argv):
    global chromosome, range_start_bp, range_end_bp, pheno_loc
    global user_script_loc, chrband, build
