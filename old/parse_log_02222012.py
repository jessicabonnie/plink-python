...
global log_folder, summary_name, run_info, freq_loc, map_loc
...
def summarize_folder(log_folder, freq_loc, map_loc, summary_name,run_info,annot_dict):
    '''
    ...
    for log_file in log_list:
        ...
        read_log(summary_file,log_file,freq_loc,map_loc, annot_dict)
...
USAGE: parse_log.py [FLAG] OBJECT
    ...
    -m, --map           path of unadapted map file                          {2}
    -s, --summary       desired name of summary file (no extension)         {3}
    -r, --runinfo       desired name of file containing SNP counts (no ext) {4}
    -h, --help          display this usage string
       
    '''.format(LOG_FOLDER,FREQ_LOC,MAP_LOC,SUMMARY_NAME,RUN_INFO))
...
def cl_arguments(argv):
    ...
    '''
    global summary_name, log_folder, run_info, freq_loc, map_loc
...
##    repair_loc = None
    ...
    try: 
        opts, args = getopt.getopt(argv, "hl:s:r:f:m:",
                                   ["help","logfolder=","summary=",
                                    "runinfo=","freq=","map=","repair-dict="])
