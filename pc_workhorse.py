...
def find_and_record(assoc_loc, SNP_loc, condition_list,plink_test,c_interval):
    ...
    with open(SNP_loc, mode = 'a') as SNP_file:     #open/create the SNP file
        snp = findSNPWithSmallestPValue(tuple_list)
        SNP_file.write(snp)
        SNP_file.write('\n')
...
def cl_arguments(argv):
    ...
    global chrband, build, interrupt
...
def make_lz_title(range_label, pheno_tag, label, ref_snp=None):
    global loopcount
    title = '{0}: {1} SNPs Conditioned Out'.format(range_label,loopcount)
    #title = '{0} Region Case/Control: {1} SNPs Conditioned Out'.format(range_label,loopcount)
    if not pheno_tag == '':
        title = title + '\n Conditioned on Expression of {0}'.format(pheno_tag)
    if not ref_snp is None:
        title = title + ' (Reference SNP: {0})'.format(ref_snp)
        #title = title + '\n ({0} Provided as LD Reference SNP)'.format(ref_snp)
    elif '~' in label:
        title = title + '\n (Initial SNP Provided)'
        #title = title + '\n (Condition List Provided at Start)'
    return title

def locuszoom(assoc_loc, chromosome,range_start_bp,range_end_bp,label, title, ref_snp=None):
    ...
    pdf_start =  os.path.join(cwd,label+'_')  
...
##                           '--build','hg18','--pop','EUR','showRecomb=FALSE'])
        ...
        lz_cl_args.extend(['--build','hg18'])
        if str(chromosome) not in ['23','24','25','26']:
            lz_cl_args.extend(['--source','1000G_June2010',
                               '--pop','CEU'])
    else:
        lz_cl_args.extend(['--source','1000G_June2010',
                           '--build','hg18','--pop','CEU'])
    if ref_snp is not None:
        lz_cl_args.extend(['--refsnp',ref_snp])
        pdf_loc = pdf_start + '{0}.pdf'.format(ref_snp.replace(':','_'))
    else:
        pdf_loc =pdf_start+'chr{0}_{1}-{2}.pdf'.format(chromosome,range_start_bp,range_end_bp)
...
def given_condition_list(SNP_loc, condition_list, interrupt):
    global loopcount
        ...
        with open(list_loc, mode = 'r') as condition_file:
            ...
            for condition in condition_file:
                if not condition.split() == []:
                    condy = condition.strip()
                    SNP_file.write(condy + '\n')
                    print('\t\t'+condy)
                    counter = counter + 1
    if interrupt:
        loopcount = counter
        print("NOW SETTING LOOPCOUNT TO COUNTER!!")
    elif counter == 1:
        ref_snp = condy
    else:
        print("THERE ARE TWO MANY SNPS IN THE STARTING CONDITION_LIST!!")
        exit(2)
##    else:
##        ref_snp = None
##        loopcount = counter
...
'''.format(counter))
    sys.stdout.flush()
    return ref_snp, counter

def fix_pdf(pdf_loc):
    if os.path.exists(pdf_loc):
        head, tail = os.path.split(pdf_loc)
    #path_dir = os.path.abspath(os.path.join(os.path.dirname(cwd),".."))
    #path_dir = os.path.dirname(outfolder)
        pdf_folder = os.path.join(os.path.dirname(head), 'lz_plots')
        if not os.path.exists(pdf_folder):
            os.makedirs(pdf_folder)
        new_pdf = os.path.join(pdf_folder, tail)
        cmd_list = ['gs','-sDEVICE=pdfwrite','-dNOPAUSE',
                    '-dBATCH','-dSAFER','-dFirstPage=1',
                    '-dLastPage=1','-sOutputFile='+new_pdf, pdf_loc]
        cmd = ' '.join(cmd_list)
        os.system(cmd)
...
'''.format(loopcount))
        ...
        if loopcount == 0 and condition_list is not None:
            ref_snp, part_counter = given_condition_list(SNP_loc, condition_list, interrupt)
            title = make_lz_title(rangelabel, pheno_tag, assoc_file_basename, ref_snp).replace('_z','').replace('_a','').replace('_b','')
            pdf_loc = locuszoom(assoc_file_name, chromosome,range_start_bp,range_end_bp,assoc_file_basename, title,ref_snp)
            fix_pdf(pdf_loc)
            ref_snp = None
...
*****************************************************************
    ...
    ****************************************************
******************************************************************
'''.format(time.strftime("%a,%c"),cband, rangelabel))
